...
"""
Planificateur des livraisons par plantation — version avancée
- Saisonnalité 60/40
- Target global (plusieurs modes)
- Fréquences aléatoires
- Calendrier & cap journalier, tolérance, exclusions
- Historique SQLite (ajout manuel + import du Suivi)
- Exports Excel (Plan + Suivi + Résumé + Paramètres)
- Fiche d’export à partir du plan
"""
...
# -------------------- Page (NE L'APPELER QU'UNE FOIS, TOUT EN HAUT) --------------------
st.set_page_config(
    page_title="Planificateur des livraisons (rendement annuel)",
    layout="wide",
    page_icon="📦",
...
if df is not None:
    ...
    # Appliquer exclusions par CODE_PLANTATION (correctement indenté)
    removed = 0
    if excl_codes:
        before = len(df)
    ...
    def plan_for_single_month(df_in, year, month_num, freq_global, freq_rand=False, freq_min=1, freq_max=3,
        ...
        if not plan.empty:
            ...
            elif order_mode == "Groupé par plantation":
                plan = plan.sort_values(["Code plantation","Date planifiée"]).reset_index(drop=True)
    ...
    if generate_clicked:
        ...
        if plan.empty:
            ...
            # Enregistrement auto dans l'historique si demandé
            if auto_book:
                count = 0
                for _, rr in plan.iterrows():
                    if insert_delivery(rr["Code plantation"], rr["Nom plantation"], rr["Coopérative"],
                                       int(rr["Année"]), int(rr["Mois"]), rr["Date planifiée"],
                                       float(rr["Quantité prévue (kg)"])):
                        count += 1
...
# Bouton de génération de la fiche (disponible si un plan existe dans la session)
